// -*- C++ -*-
//
// Package:    Phase1SiPixelMonitorRawData
// Class:      Phase1SiPixelRawDataErrorSource
// 
/**\class 
 ...
 the errors.  Uses Phase1SiPixelRawDataErrorModule class to book and fill individual folders.  
...
//
#include "DQM/Phase1SiPixelMonitorRawData/interface/Phase1SiPixelRawDataErrorSource.h"
// Framework
...
Phase1SiPixelRawDataErrorSource::Phase1SiPixelRawDataErrorSource(const edm::ParameterSet& iConfig) :
  conf_(iConfig),
...
Phase1SiPixelRawDataErrorSource::~Phase1SiPixelRawDataErrorSource()
{
   // do anything here that needs to be done at desctruction time
   // (e.g. close files, deallocate resources etc.)
  LogInfo ("PixelDQM") << "Phase1SiPixelRawDataErrorSource::~Phase1SiPixelRawDataErrorSource: Destructor"<<endl;
}

void Phase1SiPixelRawDataErrorSource::dqmBeginRun(const edm::Run& r, const edm::EventSetup& iSetup){

  LogInfo ("PixelDQM") << " Phase1SiPixelRawDataErrorSource::beginRun - Initialisation ... " << std::endl;
...
void Phase1SiPixelRawDataErrorSource::bookHistograms(DQMStore::IBooker & iBooker, edm::Run const &, edm::EventSetup const & iSetup){
...
//------------------------------------------------------------------
void Phase1SiPixelRawDataErrorSource::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  eventNo++;
  ...
  std::cout << "**** event1\n";
    ...
    if(!rawDataHandle.isValid()){
      edm::LogInfo("Phase1SiPixelRawDataErrorSource") << "inputsource is empty";
    }
    else{
  ...
  std::map<uint32_t,Phase1SiPixelRawDataErrorModule*>::iterator struct_iter;
  std::map<uint32_t,Phase1SiPixelRawDataErrorModule*>::iterator struct_iter2;
...
//------------------------------------------------------------------
void Phase1SiPixelRawDataErrorSource::buildStructure(const edm::EventSetup& iSetup){

  LogInfo ("PixelDQM") <<" Phase1SiPixelRawDataErrorSource::buildStructure" ;
  ...
  for(TrackerGeometry::DetContainer::const_iterator it = pDD->detsPXB().begin(); it != pDD->detsPXB().end(); it++){
    ...
    uint32_t id = detId();
    Phase1SiPixelRawDataErrorModule* theModule = new Phase1SiPixelRawDataErrorModule(id, ncols, nrows);
    thePixelStructure.insert(pair<uint32_t,Phase1SiPixelRawDataErrorModule*> (id,theModule));
  ...
  for (int fedId = fedIds.first; fedId <= fedIds.second; fedId++) {
    ...
    Phase1SiPixelRawDataErrorModule* theModule = new Phase1SiPixelRawDataErrorModule(id);
    theFEDStructure.insert(pair<uint32_t,Phase1SiPixelRawDataErrorModule*> (id,theModule));
...
// Book MEs
//------------------------------------------------------------------
void Phase1SiPixelRawDataErrorSource::bookMEs(DQMStore::IBooker & iBooker){
    ...
    if(modOn){
      if(!theSiPixelFolder.setModuleFolder(iBooker,(*struct_iter).first,0,isUpgrade)) {
        ...
        if(!isPIB) throw cms::Exception("LogicError")
                       << "[Phase1SiPixelRawDataErrorSource::bookMEs] Creation of DQM folder failed";
      }
    }
  ...
  //cout<<"...leaving Phase1SiPixelRawDataErrorSource::bookMEs now! "<<endl;
}

DEFINE_FWK_MODULE(Phase1SiPixelRawDataErrorSource);
